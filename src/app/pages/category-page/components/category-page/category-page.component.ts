...
export class CategoryPageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    firstValueFrom(this.activatedRoute.paramMap).then(paramMap => {
      let categoryPath = paramMap.get("categoryPath");
      this.categoryTitle = this.catalogService.getCategoryNameByCategoryPath(categoryPath);
      this.categoryPageService.loadAnSetProductFiltersRepresentations().then(ok => {
        ...
        this.categoryPageService.loadAnSetProductListPage(productFilterValues, this.paginationService.getCurrentPage(), categoryPath).then(productPage => {
  ...
  public onChangePageEvent(requestedPage: number): void {
    firstValueFrom(this.activatedRoute.paramMap).then(paramMap => {
      ...
      this.categoryPageService.loadAnSetProductListPage(filterValues, requestedPage, categoryPath).then(productPage => {
        this.paginationService.setTotals(productPage.totalPages, productPage.totalElements)
      });
    })
  }
  ...
  private subscribeNavigationEnd(): Subscription {
    return this.router.events.pipe(
      ...
      tap(event => {
        this.categoryPageService.clearProductList()
      }),
      tap(paramMap => {
        let categoryPath = paramMap.get("categoryPath");
        this.categoryTitle = this.catalogService.getCategoryNameByCategoryPath(categoryPath);
